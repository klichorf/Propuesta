...
import { prepararDatosCompartir, construirRutaSharePoint, construirNombreArchivo } from './prepararCompartir.js';

import { guardarCompartir, actualizarCompartir, eliminarCompartir } from './firebaseCompartir.js';
...
export function initCompartir(validarFormulario, generarPDF) {
  ...
  btnCompartir.addEventListener('click', async () => {
    ...
    try {
      ...
      const nombreArchivo = construirNombreArchivo(data.tipoMantenimiento, idMantenimiento);
      ...
      const resultado = await subirDocumentoCompartir(nombreArchivo, rutaCarpeta, base64, (porcentaje) =>
        actualizarProgreso(loaderTexto, fill, porcentaje),
      );
      ...
      await actualizarCompartir(idMantenimiento, resultado, nombreArchivo, rutaCarpeta);
      ...
      // Reiniciar completamente el wizard
      window.dispatchEvent(new CustomEvent('maintenance:reset'));
    } catch (error) {
