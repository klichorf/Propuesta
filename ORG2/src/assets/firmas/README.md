# Firmas

Guarda en esta carpeta las firmas que quieras insertar en el informe.

Formatos recomendados:

- PNG con fondo transparente.
- JPG o WEBP con fondo blanco.

Para que el formulario cargue la firma automaticamente al seleccionar tecnico o planta,
guarda los archivos con estos nombres:

- `jorge-leonardo-rodriguez.png`
- `pineda-agudelo-yonatan-stiven.png`
- `quevedo-ladino-mario.png`
- `luis-gabriel-rodriguez.png`
- `nestor-leonardo-rodriguez.png`
- `esneider-quintero-lozano.png`
- `gerardo-marino.png`
- `angelo-stiven-cuervo-beltran.png`
- `pinzon-guevara-william-orlando.png`
- `olga-liliana-bernal-rodriguez.png`
- `leal-herrera-jorge-andres.png`
- `henao-bedoya-maria-cristina.png`
- `ricardo-andres-bejarano.png`

Si una firma no existe, el formulario mostrara el aviso y podras firmar o insertar
la imagen manualmente.
